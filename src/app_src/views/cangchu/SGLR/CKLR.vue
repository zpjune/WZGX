<template>
  <div id="CKLR" class="app-container calendar-list-container">
    ...
    >
      <el-card>
        <el-form ref="dataForm" :model="temp" label-width="120px" style="width: 99%;">
          ...
          <el-col :span="24">
            <el-form-item label="出库时间" prop="CK_Time">
              <el-date-picker
                type="date"
                placeholder="选择日期"
...
import {
  ...
  updateCKInfo,
  getCKInfo
} from "@/app_src/api/cangchu/SGLR/CKLR";
  ...
  methods: {
    ...
    createData() {
      ...
      this.$refs["dataForm"].validate(valid => {
        if (valid) {
          createCKInfo(this.temp).then(response => {
    ...
    updateData() {
      this.$refs["dataForm"].validate(valid => {
        if (valid) {
          ...
          updateCKInfo(tempData).then(response => {
            var message = response.data.message;
            var message = "成功";
            var title = "失败";
            var type = "error";
